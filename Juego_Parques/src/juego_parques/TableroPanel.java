...
public class TableroPanel {
    ...
    // La variable duplicada debe ser eliminada.
    ...
    public void setDados (int [] tirada){ this.dados = tirada;}
    
    @Override
    protected void àintComponent(Graphics g){
      super.paintComponent(g);
      Graphics2D g2d = (Graphics2D)g;
      
      //Bases cuadradas (6x6)
      int bs = 6*tamCasilla;
      g2d.setColor (new Color(255,80,80)); // Rojo g2d.fillRect(0,0,bs,bs);
      
      g2d.setColor (new Color (80,80,255)); // Azul g2d.fillRect(9*tamCasilla,0,bs,bs);
      
      g2d.setColor (new Color (80,200,80)); // Verde g2d.fillRect (9*tamCasilla,bs,bs);
      
      g2d.setColor (new Color (255,220,80)); //Amarillo g2d.fillRect(0.9 * tamCasilla,bs,bs);
      
     
      //Meta Central (diamante)
      g2d.setColor(new Color (255,80,80));
      g2d.fillPolygon(new int []{6*tamCasilla,8*tamCasilla,7*tamCasilla},new int []{6* tamCasilla,6*tamCasilla,7*tamCasilla}, 3);
      
      g2d.setColor(new Color (80,80,255));
      g2d.fillPolygon(new int [] {8*tamCasilla,8*tamCasilla,7*tamCasilla},new int []{6* tamCasilla,8*tamCasilla,7*tamCasilla},3);
      
      g2d.setColor(new Color (80,200,80));
      g2d.fillPolygon(new int [] {8*tamCasilla,6*tamCasilla,7*tamCasilla}, new int []{8*tamCasilla,8*tamCasilla,7*tamCasilla},3);
      
      g2d.setColor(new Color (255,220,80));
      g2d.fillPolygon(new int [] {6*tamCasilla,6*tamCasilla,7*tamCasilla}, new int []{8*tamCasilla,6*tamCasilla,7*tamCasilla},3);
    ...
    for (int i = 0; i <tablero.getRuta().size();i++) 
    {
      ...
      // Dibuja el numero de la casilla
    ...
    if (jugador.getColorStr().equals("Rojo")) jugadorIndex = 0;
    else if (jugador.getColorStr().equals("Azul")) jugadorIndex =1;
    else if (jugador.getColorStr().equals("Verde")) jugadorIndex = 2;
    else if (jugador.getColorStr().equals("Amarillo"))jugadorIndex =3;
    ...
    for (Map.Entry<Point, List<Ficha>> entry : fichasPorPosicion.entrySet()) {
      ...
      g2d.setColor(Color.BLACK);
      g2d.drawOval(x + offsetX, y + offsetY, fichaSize, fichaSize);count++; 
       }
     }
   }
